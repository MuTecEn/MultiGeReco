...
def main():
    ...
    model = get_model(config["model"], config["n_class"])
    ...
    plt.title('Losses')
    plt.legend()
    plt.savefig('audio_loss.png')  # Save the plot as an image
    plt.show()  # Display the plot

    # Save the model if the validation loss has decreased
    if val_loss < best_val_loss:
        print(f'Validation loss decreased ({best_val_loss:.4f} --> {val_loss:.4f}). Saving model...')
        torch.save(model.state_dict(), 'path/to/save/folder') # Save the model's state dictionary
